...
// ✅ Render loader if products not ready
    ...
    <div className="min-h-screen bg-background p-10">
      ...
      <div className="space-y-10">
        {Object.values(groupedProducts).map(group => (
          <div key={group.parent._id} className="bg-white/60 border border-muted rounded-3xl shadow-md p-8">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-text-main">{group.parent.name}</h3>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left">
                ...
                <tbody className="divide-y divide-muted">
                  {group.items.map(p => (
                    <tr key={p._id} className="hover:bg-accent/10 transition">
                      <td className="py-4 flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-semibold text-text-main">{p.title}</p>
                        </div>
                      </td>
                      <td className="text-sm text-primary">{p.category?.name}</td>
                      <td className="font-medium text-text-main">${p.price}</td>
                      <td>
                        <span className="bg-muted px-3 py-1 rounded-full text-sm">{p.stock}</span>
                      </td>
                      <td>
                        <span
