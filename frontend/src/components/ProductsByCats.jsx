...
export default function CategoriesWithSubproducts() {
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
    {parentCategories.map((parent, index) => {
      ...
      return (
        <div key={parent._id} className="flex flex-col gap-4">
          {/* PARENT CARD */}
          <div
            onClick={() => setSelectedParent(isActive ? null : parent._id)}
          ...
          {/* SUBCATEGORIES — render directly below the parent */}
          {isActive && (
            <div className="mb-8">
              {/* MOBILE */}
              <div className="flex gap-4 overflow-x-auto md:hidden px-2">
                {getSubcategories(parent._id).map(subcat => {
                  ...
                  return (
                    <div
                      ...
                      className="min-w-[180px] rounded-xl border-2 border-[#F4A460] shadow-md hover:shadow-xl transition flex-shrink-0"
                    >
                      <div className="relative h-44 w-full overflow-hidden rounded-lg group">
                        <img
                        ...
                        <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                          <span className="text-[#F5F5DC] text-lg font-semibold text-center">
                            {subcat.name}
                          </span>
                        </div>
                      </div>
